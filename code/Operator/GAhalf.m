...
%--------------------------------------------------------------------------
    ...
    if isa(Parent(1),'INDIVIDUAL')
        ...
        aa = unidrnd(length(index));
        Guide = Parent(1,index(aa)).dec;  % 第三个目标函数最小的个体作为引导个体
        Parent = Parent.decs;
    else
        calObj = false;
    end
    ...
    switch Global.encoding
        ...
        case 'INTEGER'
            ...
            Offspring = CrossoverOperation(Parent,Global.pc,Guide,{'Uniform','POX'});

            Offspring = Offspring(1,:);

            Offspring = MutationOperation(Offspring,Global.pm,Global.ETC.AE,{'Swap'});
